...
// Fixes: gemini-1.5-flash (better free quota), Search tab, Android icon fix
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── GEMINI API (gemini-1.5-flash — melhor free tier) ────────────────────────
async function callGemini(messages, systemPrompt, geminiKey) {
  if (!geminiKey) throw new Error('Chave Gemini não configurada. Vá em Settings ⚙️');
  ...
  // gemini-1.5-flash tem free tier muito mais generoso que 2.0-flash
...
Keep messages concise and enthusiastic. Always reply in the same language the user writes in.`;
  ...
  return (
    <div className="game-card">
      ...
      <div className="game-info">
        ...
        {entry?.rating && !showRating && <div className="my-rating">Minha nota: <span>{entry.rating}/10</span></div>}
        {showRating && (
...
// ── Search View (nova aba de busca direta) ────────────────────────────────────
  ...
  return (
    <div className="search-view">
      <div className="search-header">
        <h2>🔍 Buscar Jogos</h2>
        <p className="search-sub">Pesquise e adicione jogos à sua biblioteca diretamente</p>
      </div>
      ...
      {!loading && searched && results.length === 0 && !error && (
        <div className="empty-lib">
          ...
          <p>Nenhum jogo encontrado.<br/>Tente outros termos ou filtros.</p>
        </div>
      )}
      ...
      {!loading && !searched && (
        <div className="search-placeholder">
          ...
          <p>Use os filtros acima para encontrar jogos<br/>e adicioná-los à sua biblioteca!</p>
        </div>
      )}
      ...
      {results.length > 0 && !loading && (
        <>
          ...
          <div className="games-grid">
            {results.map(g => (
              <GameCard key={g.id} game={g} library={library} onUpdateLibrary={onUpdateLibrary} />
            ))}
          </div>
        </>
      )}
  ...
  return (
    <div className="library-view">
      ...
      {items.length === 0 ? (
        <div className="empty-lib">
          <div className="empty-icon">🎮</div>
          <p>Nenhum jogo aqui ainda.<br/>Use o Chat ou a Busca para descobrir jogos!</p>
        </div>
      ) : (
        <div className="games-grid">
          {items.map(entry => (
...
function SettingsView({ settings, onSave, user, onLogout }) {
  ...
  return (
    <div className="settings-view">
      ...
      <div className="settings-section">
        <h3>🔑 Chaves de API</h3>
        <p className="settings-note">Suas chaves ficam salvas apenas no seu dispositivo. Nunca são enviadas a nenhum servidor nosso.</p>
        ...
        <div className="settings-card">
          ...
          <p className="setting-desc">
            Alimenta o chat de IA — camada gratuita disponível.<br/>
            Obtenha em <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer">aistudio.google.com</a> → clique em <strong>"Get API key"</strong>. Sem cartão de crédito!
          </p>
          <input type="password" className="setting-input" placeholder="AIza..." value={geminiKey} onChange={e => setGeminiKey(e.target.value)} />
          {geminiKey && <div className="key-status ok">✓ Chave inserida — pronto para usar o chat!</div>}
        </div>
        ...
        <div className="settings-card">
          ...
          <p className="setting-desc">Aumenta os limites de busca de jogos. Obtenha grátis em <a href="https://rawg.io/apidocs" target="_blank" rel="noreferrer">rawg.io/apidocs</a></p>
      ...
      <div className="settings-section">
        ...
        <div className="settings-card">
          <p className="setting-desc">
            <strong>Android (Chrome):</strong> Toque no menu ⋮ → "Adicionar à tela inicial" ou "Instalar app"<br/>
            <strong>iPhone (Safari):</strong> Toque em Compartilhar → "Adicionar à tela de início"<br/>
            <strong>PC:</strong> Clique no ícone ⊕ na barra de endereços
          </p>
        </div>
      </div>
  ...
  return (
    <div className="install-banner">
      ...
      <div className="install-text">
        <strong>Instalar GamePal</strong>
        <span>Adicione à tela inicial para melhor experiência</span>
      </div>
...
function App() {
  ...
  useEffect(() => {
    const onHash = () => {
      ...
      if (['chat','library','search','settings'].includes(h)) setView(h);
    };
  ...
  const sendMessage = async () => {
    ...
    try {
      const history = newMessages.slice(-12).map(m => ({ role: m.role === 'user' ? 'user' : 'assistant', content: m.content }));
      const raw = await callGemini(history, SYSTEM_PROMPT, settings.geminiKey);

      const jsonMatch = raw.match(/```json\s*([\s\S]*?)```/);
      let action = { action: 'chat', message: raw };
      if (jsonMatch) { try { action = JSON.parse(jsonMatch[1]); } catch {} }
      ...
      setMessages(prev => [...prev, {
        role: 'assistant', content: action.message || raw,
        games: games.length > 0 ? games : null, ts: Date.now(),
      }]);
    } catch (err) {
      setApiError(err.message || 'Algo deu errado');
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: '⚠️ Erro ao conectar ao Gemini. Verifique sua chave de API em Configurações ⚙️',
        ts: Date.now(),
      }]);
    } finally {
      setLoading(false);
  ...
  const SUGGESTIONS = [
    ...
    'Melhores jogos indie 2023',
    'Jogos de terror curtos',
        ...
        {view === 'chat' && (
          <div className="chat-area">
            <div className="chat-header">
              <div className="chat-header-left">
                ...
                <div>
                  ...
                  <div className="chat-status">Assistente de Jogos com IA</div>
                </div>
              </div>
              <button className="btn-clear-chat" onClick={() => { if(confirm('Limpar histórico?')) setMessages([]); }} title="Limpar chat">🗑</button>
            </div>
            ...
            <div className="messages">
              {messages.length === 0 && (
                <div className="welcome">
                  ...
                  <h2>Olá, {user.name}!</h2>
                  <p>Pergunte qualquer coisa sobre jogos. Vou te ajudar a descobrir, organizar e acompanhar sua jornada gamer.</p>
                  {!settings.geminiKey && (
                    <div className="api-key-warning">
                      ⚠️ Configure sua <button className="link-btn" onClick={() => navigate('settings')}>chave Gemini</button> nas Configurações para começar. É grátis!
                    </div>
                  )}
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`message ${msg.role}`}>
                  ...
                  <div className="msg-bubble">
                    ...
                    {msg.games && (
                      <div className="games-grid">
                        {msg.games.map(g => <GameCard key={g.id} game={g} library={library} onUpdateLibrary={updateLibrary} />)}
                      </div>
                    )}
                  </div>
            ...
            <div className="input-area">
              ...
              <div className="input-hint">Enter para enviar · Gemini AI + RAWG</div>
            </div>
          </div>
        )}
      </main>
