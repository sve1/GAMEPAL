  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (action.action === 'add' && action.gameName) {
        try {
          // Search RAWG for the game to get full info
          const results = await searchGames({ query: action.gameName, pageSize: 1 }, settings.rawgKey);
          if (results.length > 0) {
            const game = results[0];
            const status = action.status || 'backlog';
            const now = Date.now();
            const currentLib = library; // captured via closure
            const updatedLib = {
              ...currentLib,
              [game.id]: {
                ...(currentLib[game.id] || {}),
